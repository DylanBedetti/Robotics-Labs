...
void drive_to_edge(){
    ...
    if (drive_length > 150) {
        VWStraight(drive_length - 150,100);
        VWWait();
        VWCurve(1.7*150, 105, turn_rate);
        VWWait();
    }
    else {
        VWCurve(1.5 * drive_length, 105, turn_rate);
        VWWait();
    }
...
void scan(){
    ...
    if (leftsensor < rightsensor){
        VWTurn(90,50); //+ve turn is to the left
        VWWait();
        LCDClear();
